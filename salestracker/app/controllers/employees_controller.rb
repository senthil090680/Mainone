...
FROM SALE_DATA WHERE Date =#{'current_date'}");
    ...
    from_time_new=@fromtime.strftime("%d %b, %Y")
     ...
     if rsm.to_s=='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT b.customer_code,b.customer_name,b.Address_line1,b.GPS_latitude ,b.GPS_longitude FROM SALE_DATA a,CUSTOMER_MASTER b
 ...
 AND b.GPS_latitude !='' AND b.GPS_longitude != '' AND (a.Date BETWEEN '#{from_date}' AND '#{to_date}')")
     ...
     elsif  rsm.to_s!='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT h.customer_code,h.customer_name,h.Address_line1,h.GPS_latitude ,h.GPS_longitude FROM SALES_PERSONNEL a, SALE_DATA b,CUSTOMER_MASTER h
...
AND (b.Date BETWEEN '#{from_date}' AND '#{to_date}')")
               ...
               sale_data=SaleData.find_by_sql("SELECT h.customer_code,h.customer_name,h.Address_line1,h.GPS_latitude ,h.GPS_longitude FROM SALES_PERSONNEL a, SALE_DATA b,PRODUCT_MASTER c,BRAND_MASTER d,CUSTOMER_MASTER h
...
AND (b.Date BETWEEN '#{from_date}' AND '#{to_date}')")
       ...
       sale_data=SaleData.find_by_sql("SELECT h.customer_code,h.customer_name,h.Address_line1,h.GPS_latitude ,h.GPS_longitude FROM SALES_PERSONNEL a, SALE_DATA b,PRODUCT_MASTER c,BRAND_MASTER d,PRINCIPAL_MASTER e,CUSTOMER_MASTER h
WHERE a.sales_person_code = b.salesperson_code
AND b.customer_code = h.customer_code
AND b.product_code=c.product_code
AND c.brand=d.brand_code
...
AND (b.Date BETWEEN '#{from_date}' AND '#{to_date}')")
       ...
       sale_data=SaleData.find_by_sql("SELECT h.customer_code,h.customer_name,h.Address_line1,h.GPS_latitude ,h.GPS_longitude FROM SALES_PERSONNEL a, SALE_DATA b,PRODUCT_MASTER c,BRAND_MASTER d,PRINCIPAL_MASTER e,CUSTOMER_MASTER h,DIVISION_MASTER i
WHERE a.sales_person_code = b.salesperson_code
AND b.customer_code = h.customer_code
AND b.product_code=c.product_code
AND c.brand=d.brand_code
AND d.principal=e.principal_code
AND e.division_code=i.division_code
AND e.division_code='#{division}'
AND h.GPS_latitude IS NOT NULL
AND h.GPS_longitude IS NOT NULL
AND h.GPS_latitude !='' AND h.GPS_longitude != ''
AND (b.Date BETWEEN '#{from_date}' AND '#{to_date}')")
...
AND (b.Date BETWEEN '#{from_date}' AND '#{to_date}')")
     ...
     def get_total_sales_change
        ...
        division=params['division']
     if rsm.to_s=='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT  format(sum( total_values ),0) as total_values , format(sum(total_order ),0) as total_order , format(sum( line_item ),0) as line_item , format(avg( line_item ),0) as avg_line_item ,format(round(sum(total_values)/sum(total_order)),0)as drop_size
FROM SALE_DATA where (Date BETWEEN '#{from_date}' AND '#{to_date}')")
        

     elsif  rsm.to_s!='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT  format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size
 FROM SALES_PERSONNEL a, SALE_DATA b
WHERE a.supervisor = '#{rsm}'
AND a.sales_person_code = b.salesperson_code
AND (Date BETWEEN '#{from_date}' AND '#{to_date}')")
     elsif rsm.to_s=='0' and product.to_s!='0' and brand.to_s=='0' and principal.to_s=='0'and division.to_s=='0'
           sale_data=SaleData.find_by_sql("SELECT  format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size FROM SALES_PERSONNEL a, SALE_DATA b
WHERE b.product_code = '#{product}'
AND a.sales_person_code = b.salesperson_code
AND (Date BETWEEN '#{from_date}' AND '#{to_date}')")
               ...
               sale_data=SaleData.find_by_sql("SELECT  format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size FROM SALES_PERSONNEL a, SALE_DATA b,PRODUCT_MASTER c,BRAND_MASTER d
WHERE a.sales_person_code = b.salesperson_code
AND b.product_code=c.product_code
AND c.brand=d.brand_code
AND c.brand='#{brand}'
...
AND (Date BETWEEN '#{from_date}' AND '#{to_date}')")
       ...
       sale_data=SaleData.find_by_sql("SELECT  format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size FROM SALES_PERSONNEL a, SALE_DATA b,PRODUCT_MASTER c,BRAND_MASTER d,PRINCIPAL_MASTER e,DIVISION_MASTER i
WHERE a.sales_person_code = b.salesperson_code
AND b.product_code=c.product_code
AND c.brand=d.brand_code
AND d.principal=e.principal_code
...
WHERE product_code IS NOT NULL")
      ...
      def get_principal_data_division

        principal_value=params['principal']
       if principal_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT
(b.division_code),b.division_name
FROM DIVISION_MASTER b,PRINCIPAL_MASTER d
WHERE d.division_code=b.division_code
AND d.principal_code='#{principal_value}'");
       else
          brand_code=SaleData.find_by_sql("SELECT DISTINCT
(division_code),division_name
...
WHERE division_code IS NOT NULL")
     ...
     def get_brand_data_division
      brand_value=params['brand']
       if brand_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT
(i.division_code),i.division_name
FROM DIVISION_MASTER i, BRAND_MASTER c,PRINCIPAL_MASTER d
WHERE c.principal=d.principal_code AND d.division_code=i.division_code
...
def get_product_data_division
     product_value=params['product']
       if product_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT
(i.division_code),i.division_name
FROM  DIVISION_MASTER i,PRODUCT_MASTER b,BRAND_MASTER c,PRINCIPAL_MASTER d
WHERE b.brand=c.brand_code and c.principal=d.principal_code and d.division_code=i.division_code
AND b.product_code='#{product_value}'");
       else
          brand_code=SaleData.find_by_sql("SELECT DISTINCT
(division_code),division_name
...
WHERE brand_code IS NOT NULL")
     ...
     def get_division_data_product
   division_value=params['division']
       if  division_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT(b.product_code),b.product_name
FROM PRODUCT_MASTER b,BRAND_MASTER c,PRINCIPAL_MASTER d,DIVISION_MASTER i
WHERE b.brand=c.brand_code
and c.principal=d.principal_code and d.division_code=i.division_code
AND i.division_code='#{division_value}'");
       else
          brand_code=SaleData.find_by_sql("SELECT DISTINCT
(product_code),product_name
FROM  PRODUCT_MASTER
WHERE product_code IS NOT NULL")
      ...
      def get_division_data_brand
       division_value=params['division']
       if  division_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT
(c.brand_code),c.brand_name
FROM BRAND_MASTER c,PRINCIPAL_MASTER d,DIVISION_MASTER i
WHERE  c.principal=d.principal_code and d.division_code=i.division_code
AND i.division_code='#{division_value}'");
       else
          brand_code=SaleData.find_by_sql("SELECT DISTINCT(brand_code),brand_name
...
WHERE brand_code IS NOT NULL")
      ...
      render :json => response
       ...
       def get_division_data_principal
division_value=params['division']
       if division_value.to_s!='0'
       brand_code=SaleData.find_by_sql("SELECT DISTINCT
(d.principal_code),d.principal_name
FROM  PRINCIPAL_MASTER d,DIVISION_MASTER i
WHERE d.division_code=i.division_code
AND i.division_code='#{division_value}'");
       else
          brand_code=SaleData.find_by_sql("SELECT DISTINCT
(principal_code),principal_name
...
WHERE principal_code IS NOT NULL")
 ...
 def get_customer_count
     ...
     if rsm.to_s=='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT  count(*) as count,format(sum( total_values ),0) as total_values , format(sum(total_order ),0) as total_order , format(sum( line_item ),0) as line_item , format(avg( line_item ),0) as avg_line_item ,format(round(sum(total_values)/sum(total_order)),0)as drop_size
FROM SALE_DATA where (Date BETWEEN '#{from_date}' AND '#{to_date}') AND customer_code=#{customer_code}")


     elsif  rsm.to_s!='0' and product.to_s=='0' and brand.to_s=='0' and principal.to_s=='0' and division.to_s=='0'
        sale_data=SaleData.find_by_sql("SELECT  count(*) as count,format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size
 FROM SALES_PERSONNEL a, SALE_DATA b
WHERE a.supervisor = '#{rsm}'
AND a.sales_person_code = b.salesperson_code
AND (Date BETWEEN '#{from_date}' AND '#{to_date}') AND customer_code=#{customer_code}")
     elsif rsm.to_s=='0' and product.to_s!='0' and brand.to_s=='0' and principal.to_s=='0'and division.to_s=='0'
           sale_data=SaleData.find_by_sql("SELECT  count(*) as count,format(sum( b.total_values ),0) as total_values , format(sum(b.total_order ),0) as total_order , format(sum( b.line_item ),0) as line_item , format(avg( b.line_item ),0) as avg_line_item ,format(round(sum(b.total_values)/sum(b.total_order)),0)as drop_size FROM SALES_PERSONNEL a, SALE_DATA b
WHERE b.product_code = '#{product}'
AND a.sales_person_code = b.salesperson_code
AND (Date BETWEEN '#{from_date}' AND '#{to_date}') AND customer_code=#{customer_code}")
