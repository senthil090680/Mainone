...
$('#loading_data').show();
   ...
   //alldivision
        $.ajax({
          url : "/employees/get_alldivisions",
          type : "POST",
          error : function(error) {
            hideLoader('fail', 'Sorry Server down,\n Please try again later', '');
          ...
          success : function(data) {
            ...
            if(success == "1")
            {
              var division_array=data['message'];

              var all_divisions=data['message'].length;
              if (all_divisions > 0)
              {
                // alert(JSON.stringify(data['message']));

                $('#division').empty().append($('<option>', {
                  value: "0",
                  text : "ALL"
                }));
                $.each(division_array, function (i, item) {
              ...
              {
                $('#division option[value!="0"]').remove();
