...
try
{
    for (i = 0; i < sale_data.length; i++) {
      marker = new google.maps.Marker({
        position: new google.maps.LatLng(sale_data[i].GPS_latitude, sale_data[i].GPS_longitude),
		  icon: {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
         // infowindow.setContent(sale_data[i].GPS_latitude + "," + sale_data[i].GPS_longitude);
...
catch (e)
{
    ...
    function show_cluster(sale_data)
    {
...
for (var i = 0; i < sale_data.length; i++) {
        var lat = sale_data[i].GPS_latitude;
        var lng = sale_data[i].GPS_longitude;
        var latLng = new google.maps.LatLng(lat, lng);
        var marker = new google.maps.Marker({map: map, position: latLng,icon: {
          path: google.maps.SymbolPath.CIRCLE,
			    strokeColor: '#8e2014',
			    scale: 4,
				strokeOpacity: 1.0,
...
var params ={'customer_code':sale_data[i].customer_code,'from_date':from_date_format,'to_date':to_date_format,'rsm':rsm,'product':product,'brand':brand,'principal':principal,'division':division};
           $.ajax({
          url : "/employees/get_customer_count",
          data : params,
          type : "POST",
          ...
          success : function(data) {
            ...
            if(success == "1")
            {
             var customer_count=data['message'];
             ...
             var alldata=data['message'];
              ...
              infowindow.setContent('<div class=show_message_gmap1>'+'<header>'+'<span onclick="datachange();">'+'<span style="font-size:42px;font-weight:bold;padding-left:20px;color:red;cursor:pointer;"  >'+count+'</span>'+'</span>'+'</header>'+'Sales :'+total_values+'<br/>'+'Basket size :'+avg_line_item+'<br/>'+'Drop Size:'+drop_size+'</div>'+'<div class=show_message_gmap>'+'<br/>'+'Name   :   '+sale_data[i].customer_name+'</br>Address	:   '+sale_data[i].Address_line1+'</div>');
            }
            }
...
var to_date_format=$.datepicker.formatDate('yy-mm-dd', new Date(to_date));
        ...
        var principal=$("#principal").val();
        var division=$("#division").val();
        var params ={'from_date_format' : from_date_format,'to_date_format' : to_date_format,'rsm' : rsm,'principal' : principal,'brand' : brand,'product' : product,'division':division};
